...
// Test model #5a: STRATA, POSTCODE, SEIFA, SEX, AGEGP - dropping fixed effects and measurement error
...
parameters{
  vector[1] b;                                         // intercept, coef for sex, seifa_cont
...
generated quantities{
  vector[n] p = inv_logit(b[1] +
                          ...
                          a_strata[strata] +
                          a_seifa[seifa]);
  array[n] real y_rep = bernoulli_rng(p);
  real p_mean = mean(p);
