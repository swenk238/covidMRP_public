...
data {
  ...
  int<lower=0> n_seifa;      // number of seifa groups
  ...
  array[J] int<lower=0,upper=1> sex_pop;           // poststratification cells, strata
  array[J] int<lower=1,upper=n_age> agegp_pop;               // poststratification cells, agegp
  array[J] int<lower=1,upper=n_seifa> seifa_pop;               // poststratification cells, agegp
...
model {
  ...
  if (!prior_only) {
   // initialize linear predictor term
    ...
    target += bernoulli_logit_lpmf(Y | logit_p_samp);
  }
...
generated quantities {
    ...
    for (j in 1:J){
      mu_pop[j] += r_1_1[agegp_pop[j]] * Z_1_1[j] +
      r_2_1[postcode_pop[j]] * Z_2_1[j] + 
      r_3_1[seifa_pop[j]] * Z_3_1[j] + 
      r_4_1[strata_pop[j]] * Z_4_1[j];
    }
    p_pop = inv_logit(mu_pop + X_pop_sex_seifa*b);
