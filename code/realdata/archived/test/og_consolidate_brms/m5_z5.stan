...
data {
  ...
  int<lower=1> n_age; // number of grouping levels
  array[n] int<lower=1> agegp; // grouping indicator per observation
  ...
  matrix[J, K] X_pop_sex_seifa; // poststrat design matrix
...
parameters {
  ...
  array[1] vector[n_age] z_age; // standardized group-level effects
  ...
  vector<lower=0>[1] sigma_age; // group-level standard deviations
...
transformed parameters {
  vector[n_age] a_age; // actual group-level effects
...
model {
  ...
  spec ~ beta(TN+1, FP+1); 
  sens ~ beta(TP+1, FN+1);    
  z_age[1] ~ normal(0, 1);                        // random effects
  z_postcode[1] ~ normal(0, 1);                        // random effects
  z_strata[1] ~ normal(0, 1);                        // random effects
  z_seifa[1] ~ normal(0, 1);                        // random effects
  
  sigma_age ~ student_t(3, 0, 2.5); // lower limit set when declaring sigma_age, alternatively, T[L,]
