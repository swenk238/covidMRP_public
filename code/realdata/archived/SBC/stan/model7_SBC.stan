...
// Test model #7: Interaction between SEIFA, SEX, AGEGP with STRATA
...
data {
  ...
  int<lower=0> n_seifa_strata;                         // number of SEIFA categories x strta categories
  int<lower=0> n_sex_strata;                          // number of SEIFA categories x agegp categories
  int<lower=0> n_agegp_strata;                            // number of sex categories x agegp 
  ...
  array[n] int seifa_sex;  // for two-way interaction effects
  array[n] int seifa_agegp;
  array[n] int sex_agegp;
  array[J] int seifa_sex_pop;  // interaction effects for poststrat table
  array[J] int seifa_agegp_pop;  
  array[J] int sex_agegp_pop; 
  array[n] int seifa_strata;  // for interaction effects with strata
  array[n] int sex_strata;
  array[n] int agegp_strata;
  array[J] int seifa_strata_pop;  
  array[J] int sex_strata_pop;  
  array[J] int agegp_strata_pop; 
}
parameters{
  ...
  real<lower=0> sigma_seifa_strata;   
  real<lower=0> sigma_sex_strata;   
  real<lower=0> sigma_agegp_strata;   
  ...
  vector<multiplier=sigma_seifa_strata>[n_seifa_strata] a_seifa_strata; 
  vector<multiplier=sigma_sex_strata>[n_sex_strata] a_sex_strata; 
  vector<multiplier=sigma_agegp_strata>[n_agegp_strata] a_agegp_strata; 
}
model{
  if (!prior_only) {
  vector[n] p = inv_logit(b[1] + 
                          ...
                          a_seifa_strata[seifa_strata] +
                          a_sex_strata[sex_strata] +
                          a_agegp_strata[agegp_strata]);
  ...
  a_seifa_strata ~ normal(0, sigma_seifa_strata);
  a_sex_strata ~ normal(0, sigma_sex_strata);
  a_agegp_strata ~ normal(0, sigma_agegp_strata);
  ...
  sigma_seifa_strata ~ normal(0, coef_prior_scale);  
  sigma_sex_strata ~ normal(0, coef_prior_scale);  
  sigma_agegp_strata ~ normal(0, coef_prior_scale);  
}
generated quantities{
  ...
  for (j in 1:J){
      p_pop[j] = inv_logit(b[1] +
                           ...
                           a_seifa_strata[seifa_strata_pop[j]] +
                           a_sex_strata[sex_strata_pop[j]] +
                           a_agegp_strata[agegp_strata_pop[j]]);
  }
  ...
  for (i in 1:n) {
    y_pred[i] = bernoulli_rng(p_sample[i]);
    joint_log_lik += bernoulli_lpmf(y[i]|p_sample[i]);
    joint_log_lik += bernoulli_lpmf(y_pred[i]|p_sample[i]);
  }
}
