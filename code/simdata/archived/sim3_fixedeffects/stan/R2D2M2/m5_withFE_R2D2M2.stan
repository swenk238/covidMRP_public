// adapted from exp4/estSpec/non-centered/m5_estSpec_noncent.stan
...
parameters{
  vector[2] zb;  // unscaled coefficients
  ...
  real<lower=0> sigma_x5;                             // scale parameters for random effects
  ...
  vector<multiplier=sigma_x5>[n_x5] a_x5;           // varying intercepts for random effects
}
transformed parameters {
  vector[2] b;  // scaled coefficients
  vector<lower=0>[2] sdb;  // SDs of the coefficients
...
model {
  ...
  a_x5 ~ normal(0, sigma_x5);                        // random effects
  b[1] + b[2]*mean(x3_con) ~ logistic(0,1);                // prior on centred intercept, corresponds to U(0,1)
  ...
  sigma_x5 ~ normal(0, coef_prior_scale);           
}
